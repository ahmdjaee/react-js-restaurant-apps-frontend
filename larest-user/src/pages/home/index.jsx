...
export async function loader() {
  const specialMenus = await axiosClient.get("/menus?tags=Special");
  const categories = await axiosClient.get("/categories");
  return {
    specialMenus: specialMenus?.data?.data,
    categories: categories?.data?.data,
  };
}

export default function Home() {
  const { specialMenus, categories } = useLoaderData();
    ...
    <div className="overflow-x-clip">
      <section className="container justify-between flex flex-col sm:flex-row items-center sm:px-3 sm:pt-8 pb-16">
        <img
          ...
          alt=""
          className="sm:hidden mb-10  h-80"
        />
        <div className="sm:w-2/5 animate-left-slide-in text-center sm:text-start px-3 sm:px-0">
          ...
          <p className="text-base my-10">
            Enjoy a unique blend of tradition and innovation in every dish,
            delivering flavors that excite and leave a lasting impression.
          </p>
          <Link to={"/menus"}>
      ...
      <section className=" py-12 text-center">
        <div className="container">
          <h1 className="text-3xl sm:text-5xl font-semibold my-5">
            Our Special Dishes
          </h1>
          <p className="sm:w-1/2 mx-auto mb-16">
            Experience the perfect blend of tradition and innovation with our special
            dishes. Each creation is designed to excite your taste buds and leave a
            lasting impression.
          </p>
          <CartMenuLayout>
          ...
          <div className="flex flex-col mt-20 container">
            ...
            <CartMenuLayout>
              {categories.map((category) => (
                <div
                  key={category.id}
                  className="bg-white rounded cursor-pointer shadow"
                >
                  <div
      ...
      <section className="flex flex-col sm:flex-row container items-center py-12 gap-3 sm:gap-5">
        <div className="mx-5 sm:mx-0">
          <h1 className="text-3xl text-center sm:text-start sm:text-5xl font-semibold">
            Our Expert Chefs
          </h1>
          <p className="text-base my-8 sm:my-16">
            Discover the culinary talents of our skilled and experienced chefs.
          </p>
